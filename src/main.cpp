...
uint256 hashGenesisBlock("0x819c7555159ada979cb4a20960b5673bf9265b940a562814ff194ee333406419");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // YorkCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "YorkCoin Signed Message:\n";
...
{
    // YorkCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan =  60 * 60; // YorkCoin: 
static const int64 nTargetSpacing = 60 ; // YorkCoin: 
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // YorkCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // YorkCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // YorkCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xb0d90307d13a39ab5526a568b0c0b33b0395213b02f654859bcb410a7a4f32a0");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "GetHash is a software nLockTime system scriptPubKey by Satoshi Nakamoto in 2008.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1455540414;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2094574436;

        if (fTestNet)
        {
            block.nTime    = 1393739146;
            block.nNonce   = 385476762;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x98bac0f8a74b91ca118b09fbba114882c82097af86b828f7fc96ff5d14491e2d"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // YorkCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("YorkCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("YorkCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static YorkCoinMiner(CWallet *pwallet)
{
    printf("YorkCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("YorkCoin-miner");
    ...
    try { loop {
        ...
        printf("Running YorkCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("YorkCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&YorkCoinMiner, pwallet));
}
